...
const SecurityTabs: React.FC<{ scores: ScoresType }> = ({ scores }) => {
    ...
    const sortedPFs = [...((scores.cweProductFactors ?? []) as PF[])]
        .sort((a: PF, b: PF) => (a.value ?? 0) - (b.value ?? 0));
    ...
    const normalizeFixed = (v: any): 'fixed' | 'notfixed' | '' => {
        if (v === true || String(v).toLowerCase() === 'true' || String(v).toLowerCase() === 'fixed') return 'fixed';
        if (v === false || String(v).toLowerCase() === 'false' || String(v).toLowerCase() === 'not fixed') return 'notfixed';
    ...
    tabs.push({
        ...
        content: (
            <Box className="st-root">
                <h3 className="st-h3"># of CWE Pillars: {scores.vulnerabilitySummary?.cweCount ?? 0}</h3>
                                        ...
                                        <div className="measure-list">
                                            <ul>
                                                {pf.measures
                                                    ...
                                                    .sort((a: Measure, b: Measure) => Number(a.score ?? 0) - Number(b.score ?? 0))
                                                    .map((measure: Measure, idx: number) => (
                                                        <li
                                                            key={idx}
                                                            className="measure-item"
                                                            style={{ backgroundColor: setBackgroundColor(measure.score) }}
                                                        >
    ...
    tabs.push({
        ...
        content: (
            <Box className="st-root">
                <h3 className="st-h3"># of CVEs: {scores.vulnerabilitySummary?.cveCount ?? 0}</h3>
                    ...
                    return (
                        <Box key={pf.name}>
                            {filtered.map((cve: CVEItem) => (
                                <Box key={cve.name} className="cve-card">
                                    ...
                                    <ul className="cve-list">
                                        ...
                                        <li><strong>Tools used:</strong> {cve.byTool.map((t: any) => t.tool).join(', ')}</li>
                                    </ul>
        ...
        <>
            ...
            {popoutKey && (
                <div className="densityPlot">
                    ...
                    {(() => {
                        const pf = (scores?.cweProductFactors as PF[] | undefined)
                            ?.find((p: PF) => p.name === popoutKey.pfName);
