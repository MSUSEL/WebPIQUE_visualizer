...
export type ProjectFileLoadHandle = {
  openAddFile: () => void;
};
...
const ProjectFileLoad = forwardRef<ProjectFileLoadHandle, ProjectFileLoadProps>(
  (
    {
      projectId,
      onScores,
    ...
    async function handleFiles(fileList: FileList | null) {
      ...
      try {
        ...
        for (const file of files) {
          ...
          const aspects: AspectItem[] = Array.isArray(out?.aspects ?? out?.scores?.aspects)
            ? (out?.aspects ?? out?.scores?.aspects).map((a: any) => {
              if (Array.isArray(a))
    ...
    const displayList = useMemo(() => {
      ...
      const colSize = Math.ceil(sorted.length / 2);
      const left = sorted.slice(0, colSize);
      const right = sorted.slice(colSize);
    ...
    return (
      <section className="w-full">
        ...
        <div className="w-full rounded-lg border border-[#ccc] bg-white p-3">
          <div>
          ...
          >
            {displayList.map((s) => {
                ...
                return (
                  <li
                    key={s.id}
                    className={`inline-flex items-center gap-1 ${isSel
                      ? "rounded-md bg-[rgba(59,130,246,0.12)] font-semibold"
                      : ""
                      }`}
                  >
                    <input
                    ...
                    <button
                      type="button"
                      className="cursor-pointer px-0.5 text-[0.95rem]"
...
export default ProjectFileLoad;
