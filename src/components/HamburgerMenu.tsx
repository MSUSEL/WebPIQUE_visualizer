...
const FileInput = ({
  ...
  <div className="file-input">
    <label>{label}</label>
    <input type="file" accept=".json" onChange={onChange} ref={inputRef} />
  </div>
);
...
const HamburgerMenu = () => {
  ...
  const readJsonFile = (
    file: File,
    onOk: (obj: { filename: string; data: any }) => void
  ) => {
  ...
  // left/right uploads now produce { filename, data }
      ...
      <div className="menu-container">
        ...
        {isOpen && (
          <div className="menu">
            ...
            <div
              className={`menu-item ${
                activeMenuItem === "compare" ? "active" : ""
              }`}
              onClick={() => {
                setShowCompareSubmenu(!showCompareSubmenu);
                setActiveMenuItem(
                  activeMenuItem === "compare" ? null : "compare"
                );
              }}
            >
              Compare
